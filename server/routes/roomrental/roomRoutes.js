...
// router.post("/bookings", upload.single("idDocument"), createBooking);
...
["createBooking", "checkAvailability", "getBooking", "confirmPayment"].forEach((fn) => {
...
// BOOKING route
// Expect multipart/form-data with idDocument file field and other fields in body
// Protect the route so req.user is available to bookingsController.createBooking
router.post("/bookings", protect, upload.single("idDocument"), async (req, res, next) => {
  try {
    // Forward to bookings controller
    return bookingsController.createBooking(req, res, next);
  } catch (err) {
    next(err);
  }
});
...
// Example: GET /api/rooms/:id/availability?startDate=...&endDate=...
