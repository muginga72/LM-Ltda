...
function formatLocation(loc) {
  // loc can be a string or an object like { address, city, region, country }
  if (!loc) return "";
  if (typeof loc === "string") return loc;
  if (typeof loc === "object") {
    // prefer a human readable combination
    const parts = [];
    if (loc.address) parts.push(loc.address);
    if (loc.city) parts.push(loc.city);
    if (loc.region) parts.push(loc.region);
    if (loc.country) parts.push(loc.country);
    // if coordinates only, show lat,lng
...
function EditButton({ onEdit, room }) {
  return (
    <button
      type="button"
      className="btn btn-sm btn-outline-secondary me-2"
      onClick={() => onEdit(room)}
      aria-label={`Edit ${room?.roomTitle || room?.title || "room"}`}
    >
      Edit
    </button>
  );
...
export default function RoomCard({ room, onEdit, onDelete, onView, isAdmin }) {
  ...
  const bathrooms = room?.bathrooms ?? room?.bathroomCount ?? room?.numBathrooms ?? "—";
  ...
  const locationText = formatLocation(room?.roomLocation ?? room?.location ?? room?.address);
