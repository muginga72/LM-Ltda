...
export default function BookingForm({ room, onSuccess, onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!startDate || !endDate || !dateOfBirth || !idFile) {
      setStatus({ variant: "danger", text: "Please fill all fields and upload ID/passport." });
      return;
    }
    ...
    try {
      ...
      onSuccess && onSuccess(res.booking || res);
    } catch (err) {
      setStatus({ variant: "danger", text: err.message || "Booking failed." });
    } finally {
      setLoading(false);
    }
  };
